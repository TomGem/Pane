...
- Use the **user menu** (person icon) to view your account, storage usage, access the admin panel (admins only), or sign out.
- **Change your password** from the **Settings** overlay (gear icon) — enter your current password and a new one.
...
- Share by entering the other user's **email address** and choosing a permission level: **Can edit** (read-write) or **View only** (read-only).
- The shared user sees the space in their **"Shared with me"** section on the dashboard.
